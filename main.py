        ...
        if os.path.isdir(path):
            ...
            for f in fs:
                ...
                if False:  # mim.find("video")!=-1:
                    body += "<h3>%s</h3><video title='%s' controls ><source src=\"%s\" type=\"%s\"></video></br>" % (
                    p, p, urllib2.quote(t), mim)
                else:
                    body += "<a href='%s'><b>%s</b><i>(%s)</i></a></br>" % (
                    urllib2.quote(t)[1:], p, m_size(os.path.getsize(t)))
            body += "</body></html>"
...
def main():
    ...
    for i in range(0, args.concurrency-1):
        ...
        if pid == 0:  # child process
            ...
            _logger.info("starting worker-[%s]", pid)
    while True:
